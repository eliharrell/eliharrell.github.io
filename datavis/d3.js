...
transform the many statements at the end like:
  ...
  ...
which are used in pre-ES6 CommonJS modules, into the form of modern ES6 modules:
  export {
     ...
     foo,
     barThing as bar,
...
As 16 April 2024, GLK's script also injects code for the (single-cone)
HSV colorspace https://en.wikipedia.org/wiki/HSL_and_HSV so that it can
co-exist alongside the (double-cone) HSL space already in d3.
There is a separate (seemingly unmaintained) library for HSV:
  https://github.com/d3/d3-hsv
but GLK feels that given their long shared history, HSV must exist as a
functional peer to HSL wherever HSL is defined, with zero extra work.
*/
...
var reI = "\\s*([+-]?\\d+)\\s*",
    ...
    reHsvPercent = new RegExp(`^hsv\\(${reN},${reP},${reP}\\)$`),
    reHsvaPercent = new RegExp(`^hsva\\(${reN},${reP},${reP},${reN}\\)$`),
...
function color_formatHsv() {
  return hsvConvert(this).formatHsv();
}
...
function hsvConvert(o) {
  ...
  o = o.rgb();
  // copying guts of hsvConvert of https://unpkg.com/browse/d3-hsv@0.1.0/build/d3-hsv.js
  var r = o.r / 255,
    g = o.g / 255,
    b = o.b / 255,
    min = Math.min(r, g, b),
...
// copy-paste of define(Hsl) below
define(Hsv, hsv$2, extend(Color, {
  brighter(k) {
    k = k == null ? brighter : Math.pow(brighter, k);
  ...
  rgb() { // hsvPrototype.rgb of https://unpkg.com/browse/d3-hsv@0.1.0/build/d3-hsv.js
    var h = isNaN(this.h) ? 0 : this.h % 360 + (this.h < 0) * 360,
        ...
        m = v - c;
    // repackaging of hsv2rgb of https://unpkg.com/browse/d3-hsv@0.1.0/build/d3-hsv.js
    const hsv2rgb = (r1, g1, b1, m, a) => new Rgb((r1 + m) * 255, (g1 + m) * 255, (b1 + m) * 255, a);
  ...
  clamp() {
    return new Hsv(clamph(this.h), clampt(this.s), clampt(this.v), clampa(this.opacity));
  },
  displayable() {
  ...
  formatHsv() {
    const a = clampa(this.opacity);
    return `${a === 1 ? "hsv(" : "hsva("}${clamph(this.h)}, ${clampt(this.s) * 100}%, ${clampt(this.v) * 100}%${a === 1 ? ")" : `, ${a})`}`;
  }
}));
...
function hsv(hue) {
  return function(start, end) {
    var h = hue((start = hsv$2(start)).h, (end = hsv$2(end)).h),
